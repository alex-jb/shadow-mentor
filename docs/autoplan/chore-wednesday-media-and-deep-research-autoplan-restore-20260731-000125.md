...
Captured: 2026-07-31T04:01:25Z | Branch: chore-wednesday-media-and-deep-research | Commit: 8b90326

## Re-run Instructions
1. Copy 'Original Plan State' below back to docs/APP_DESIGN_BRIEF_2026-07-30.md
2. Invoke /autoplan
...
Anchored on GitHub Primer semantics + Linear monochrome discipline. **Accent = indigo `#5E6AD2`** (swap to Radix Blue `#3E63DD` if indigo reads too "modern" for a bank buyer). Accent appears ONLY on primary button / active nav / focus ring / links / selected-row rail — **never behind data, never as a fill under numbers.**
> **⚠ Decision to reconcile:** the shipped worked-example page + Kimi site currently use **teal `#12a594`**. Two coherent options — (a) adopt D's **indigo/Radix-Blue** as the system accent and migrate the teal seed, or (b) keep **teal** as the single accent and apply every other rule here unchanged. Either is fine; the rule that matters is *one accent, rationed*. Recommend deciding at the design-token pass (§8) — leaning (a) Radix Blue for bank-buyer sobriety, but teal is a legitimate hold if we want continuity. Not a blocker.
...
## 4 · Borrowed IA + case-detail patterns  ← [RESEARCH STREAM A: RegTech UI teardown]
_Canonical compliance-tool IA + 5–8 named, borrowable patterns (Vanta/Persona/Alloy/Hummingbird/etc.) — fill from A._
...
4. Notice goes out within 30 days. Generic language fails: **CFPB Circular 2022-03** — "a creditor's lack of understanding of its own methods is not a cognizable defense"; "did not meet our proprietary model" is non-compliant. Complexity is no excuse.
...
| "**The lender remains liable — Circular 2022-03: not understanding your own model is not a defense.** Shadow closes that gap." | "Set-and-forget / fully automated compliance" (regulators say automation doesn't shift responsibility). |
...
**Meta-rule for all app copy + empty states:** this buyer is defensive, personally exposed, regulation-fluent. Trusts **specific citations, admissions of the lender's own liability, examiner vocabulary**; distrusts **superlatives, "AI magic," any framing that implies the tool absorbs their accountability.** The enforcement headlines that keep them up (Vystar $1.5M CMP Oct 2024; Mass. AG July 2025 AI-underwriting disparate-impact settlement) are the fear to *mirror*, never dismiss.
