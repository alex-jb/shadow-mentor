...
|---|---|---|---|---|
| **OpenBB** | real market-data layer | AGPLv3 | **ADOPT (isolated)** | The provider boundary now exists in code: `demos/spatial-finance/providers/` — `MockMarketDataProvider` (live, the fixture) + `OpenBBMarketDataProvider` (documented AGPL-isolated stub) behind a `getProvider()` factory + `PORTFOLIO_CONTRACT`. Remaining: implement the stub against an OpenBB service (process/API isolation, AGPL must not touch Shadow core) — needs install + key (Alex-gated). |
...
- **OTel GenAI/MCP adapter** (`packages/adapter-otel`) — any instrumented agent's spans → signed Shadow evidence. Tested round-trip (6); runnable `example.js` (5-span loan-review trace → signed → tamper-caught); **`bin/otel-to-bundle.mjs` CLI** turns exported spans into a `bundle.json` that the **shipped `bin/shadow-verify.mjs`** accepts (cross-tool loop, trust SELF_SIGNED, tamper → exit 1). [queue #1 ✓]
