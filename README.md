...
- 723/724 tests green (1 skipped: OCR live smoke gated on billing envelope); GitHub Actions CI 100+ consecutive commits green
- **Cross-vertical persona pack (v0.2 LIVE)** — `lib/personas/trader-pack/` Risk Sizer wired into `POST /api/deliberate` via `{"mode": "trading", "trade": {...}}`. 7 pure-JS contract tests parallel to Orallexa Python + 10 HTTP-boundary tests enforcing the FinPos "never emit direction" invariant end-to-end. Byte-identical wire format so banking + trading + data-science verdicts will share one attestation surface once v0.4 hash-chain lands.
