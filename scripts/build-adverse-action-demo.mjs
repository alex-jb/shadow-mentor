...
// The browser verifier — byte-for-byte the verify.html logic (WebCrypto Ed25519 + hash-chain).
const VERIFY_JS = `
...
async function verifyBundle(bundle,pem){
  ...
  return {ok:true,keyId:sig.key_id,batchRoot:root};
}`;
...
async function run(){
  ...
  if(r.ok){statusEl.className="ok";statusEl.textContent="✓ Record verified — signature + hash-chain intact (key "+(r.keyId||"?")+"). The decision above is exactly what was sealed.";}
...
document.getElementById("tamper").onclick=async()=>{
  ...
  // corrupt the sealed council-verdict event's payload_hash → breaks the chain
  const ev=working.events.find(e=>e.payload_ref&&e.payload_ref.kind==="council_verdict")||working.events[2];
  ev.payload_hash=(ev.payload_hash||"").replace(/.$/, c=>c==="0"?"1":"0");
  await run();
