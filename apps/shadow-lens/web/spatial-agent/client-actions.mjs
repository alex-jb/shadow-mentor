...
export const CLIENT_ACTIONS = {
  ...
  focus_claim: { id: "claim_id" },
  focus_node: { id: "object_id" },
  move_camera_to_node: { id: "object_id" },
  open_document: {},
  open_source_mode: {},
  open_risk_mode: {},
  open_review_mode: {},
...
export function validateAction(action, scene) {
  if (!action || typeof action.name !== "string") return { ok: false, error: "action.name required" };
  const spec = CLIENT_ACTIONS[action.name];
  if (!spec) return { ok: false, error: `unknown action "${action.name}"` };
  ...
  if (extra.length) return { ok: false, error: `unexpected args: ${extra.join(",")}` };

  if (spec.id) {
    const id = args[spec.id];
    if (typeof id !== "string" || !id) return { ok: false, error: `${action.name}: ${spec.id} required` };
    // referenced object must exist in the real scene graph (no invented ids)
    if (!sceneHasObject(scene, id)) return { ok: false, error: `${action.name}: "${id}" not in the scene` };
    return { ok: true, action: { name: action.name, args: { [spec.id]: id } } };
  }
  return { ok: true, action: { name: action.name, args: {} } };
}
