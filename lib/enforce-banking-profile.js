...
import { validateReviewerInteraction, summarizeReviewerInteraction } from "./reviewer-interaction.js";
...
function runCheck(field, bundle, { payloads, verified }) {
  ...
  switch (c.kind) {
    ...
    case "reviewer_interaction_verified": {
      // Verify the reviewer actually interacted, not just that an approval event exists.
      // Needs the human_approval payload; without it we say so honestly (unknown).
      if (!payloads) return { status: "unknown", detail: "supply the human_approval payload to verify reviewer-interaction detail (decision/duration/override rationale)" };
      for (const e of bundle.events || []) {
        if (e.event_type !== c.event_type) continue;
        const p = payloads[e.seq] ?? payloads[e.payload_ref];
        const ri = p?.reviewer_interaction;
        if (ri) {
          const v = validateReviewerInteraction(ri);
          return v.valid
            ? { status: "present", detail: summarizeReviewerInteraction(ri) }
            : { status: "missing", detail: `reviewer_interaction invalid: ${v.errors[0]}` };
        }
      }
      return { status: "missing", detail: `no reviewer_interaction in any ${c.event_type} payload` };
    }
