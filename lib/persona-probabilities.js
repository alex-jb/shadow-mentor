 ...
 */
export function probabilityDistribution(verdict, absMargin, tilt) {
...
export function creditFundamentalsProbabilities(loan) {
  ...
  if (mFico < 0) return probabilityDistribution("block", -mFico);
  if (mDti < 0) return probabilityDistribution("escalate", -mDti, "block");
  // approve confidence is governed by the weakest passing margin
  return probabilityDistribution("approve", Math.min(mFico, mDti));
}
...
export function riskOfficerProbabilities(loan, risk, riskBudgetStatus) {
  ...
  if (riskBudgetStatus === "breach") return probabilityDistribution("block", Math.max(-mVar, 0.5));
  if (riskBudgetStatus === "escalate") return probabilityDistribution("escalate", Math.max(Math.abs(mVar), 0.25), "block");
  if (mLtv < 0) return probabilityDistribution("escalate", -mLtv, "block");
  return probabilityDistribution("approve", Math.min(mVar, mLtv));
}
...
export function fairLendingProbabilities(loan) {
  return loan.fair_lending_review_flag
    ? probabilityDistribution("block", BINARY_MARGIN)
    : probabilityDistribution("approve", BINARY_MARGIN);
}
...
export function customerAdvocateProbabilities(loan) {
  ...
  if (n === 0) return probabilityDistribution("approve", BINARY_MARGIN);
  return probabilityDistribution("escalate", n / MARGIN_SCALES.aa_reasons, "approve");
}
...
export function macroContrarianProbabilities(loan) {
  ...
  return cre
    ? probabilityDistribution("escalate", BINARY_MARGIN, "block")
    : probabilityDistribution("approve", BINARY_MARGIN);
}


/** AML/KYC Investigator (opt-in 6th voice) — tiered flags.
 *  Hard-block findings (sanctions hit, unverified identity) are decisive
 *  policy → saturated margin. Escalation sharpens with the finding count.
 *  Clean file → saturated approve. Same argmax===verdict guarantee. */
export function amlKycProbabilities({ hardBlockCount, escalateCount }) {
  if (hardBlockCount > 0) return probabilityDistribution("block", BINARY_MARGIN + hardBlockCount - 1);
  if (escalateCount > 0) return probabilityDistribution("escalate", escalateCount, "block");
  return probabilityDistribution("approve", BINARY_MARGIN);
}
