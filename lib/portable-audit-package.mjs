...
export const SUPPORTED_PACKAGE_VERSIONS = Object.freeze([PACKAGE_VERSION, PACKAGE_VERSION_1_1]);
...
// Supersession relation + marker vocabularies (closed). The marker is a NEUTRAL
// fixture-only successor label — it encodes no Human Review / Approval / Rejection
// semantics; those belong to a future bounded increment with new vocabulary entries.
...
// Which predecessor contract versions a 1.1 successor may reference.
...
export const FAILURE_CODES = Object.freeze([
  ...
  "SUPERSESSION_MALFORMED",    // 1.1 supersedes block missing/ill-shaped/unknown relation or marker
  "SELF_REFERENCE",            // 1.1 package claims itself as its own predecessor
]);
...
export function verifyPackageDir(dir, { publicKeyPem = null } = {}) {
  ...
  // ced8c2c baseline; every 1.1-only check is gated on isV11 so 1.0 semantics never move.
  ...
  // 3b · supersedes block — 1.1 only (mandatory there); 1.0 gets ZERO new checks here.
  if (isV11) {
    const supErrs = [];
    const sup = manifest.supersedes;
    if (typeof sup !== "object" || sup === null || Array.isArray(sup)) {
      supErrs.push("a 1.1 manifest must carry exactly one supersedes object");
    } else {
      ...
      if (!SUPERSESSION_MARKERS.includes(sup.marker)) supErrs.push(`supersedes.marker ${JSON.stringify(sup.marker)} is not in the closed vocabulary (${SUPERSESSION_MARKERS.join(", ")})`);
    ...
    if (!manifest.capability_boundary.includes("SUPERSESSION_IS_NOT_GLOBAL_LATEST")) {
      addFail("SUPERSESSION_MALFORMED", "a 1.1 package must declare SUPERSESSION_IS_NOT_GLOBAL_LATEST in capability_boundary");
    }
  ...
  // provenance schema is manifest-version-bound: 1.0 → provenance/1.0, 1.1 → provenance/1.1.
  const roleSchemas = { ...ROLE_SCHEMAS, provenance: [isV11 ? PROVENANCE_SCHEMA_1_1 : PROVENANCE_SCHEMA] };
  ...
  if (isV11) {
    // 1.1 only: the provenance member must repeat the signed supersedes block verbatim
    // (1.0 provenance content is deliberately NOT inspected — baseline behavior preserved).
    const provPath = byRole.get("provenance")[0].path;
    const prov = parseMember(provPath, "provenance");
    if (prov) {
      if (prov.schema_version !== PROVENANCE_SCHEMA_1_1) addFail("BINDING_MISMATCH", "provenance member schema_version != declared");
  ...
  const summary = {
    ...
    ...(isV11 ? { supersedes: manifest.supersedes } : {}),
  };
