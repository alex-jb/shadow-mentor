...
export function runLoanCouncil(rawLoan) {
  ...
  voices.push({
    ...
    confidence: 0.82,
    rationale: !fico_pass
      ? `FICO=${loan.credit_score} below Addendum A floor ${LOAN_DEFAULTS.fico_approve_floor} (Reg B §1002.6 permissible consideration); credit-eligibility floor failure is a hard block per Levitchi 2026-06-19 policy semantics. Principal reason under Reg B §1002.9(b)(2): insufficient credit history depth (AA01).`
      : !dti_pass
  ...
  voices.push({
    ...
    rationale: loan.fair_lending_review_flag
      ...
      : "ECOA/Reg B disparate-impact review complete; no protected-basis proxy features detected in feature set; adverse-action reasons drawn from signed dictionary per Reg B §1002.9(b)(2) principal-reason accuracy.",
  ...
  voices.push({
    ...
    rationale: advocate_ok
      ? "Customer-facing explanation and adverse-action reason list reviewed against Reg B §1002.9(b)(2) specific-reason readability; no principal-reason accuracy or dictionary coverage flags."
      : `Adverse-action reasons present (${loan.adverse_action_reasons.length}); each reason must satisfy Reg B §1002.9(b)(2) specificity — 'internal standards' or broad-bucket phrasing is insufficient. Escalate for advocate review of borrower-facing wording.`,
