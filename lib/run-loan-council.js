...
export function runLoanCouncil(rawLoan) {
  ...
  voices.push({
    ...
    confidence: 0.82,
    rationale: !fico_pass
      ? `FICO=${loan.credit_score} below Addendum A floor ${LOAN_DEFAULTS.fico_approve_floor}; credit-eligibility floor failure is a hard block per Levitchi 2026-06-19 policy semantics.`
      : !dti_pass
      ? `FICO=${loan.credit_score} OK; DTI=${loan.debt_to_income.toFixed(2)} above Addendum B ceiling ${LOAN_DEFAULTS.dti_approve_ceiling} — escalate for compensating factors / income verification.`
      : `FICO=${loan.credit_score} ≥ ${LOAN_DEFAULTS.fico_approve_floor} (Addendum A) and DTI=${loan.debt_to_income.toFixed(2)} ≤ ${LOAN_DEFAULTS.dti_approve_ceiling} (Addendum B); fundamentals acceptable.`,
  ...
  const rcdCheck = enforceReasonCodesInDictionary(adverse_action_codes);
