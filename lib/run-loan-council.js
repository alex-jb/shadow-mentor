...
import {
  creditFundamentalsProbabilities, riskOfficerProbabilities, fairLendingProbabilities,
  customerAdvocateProbabilities, macroContrarianProbabilities, roundProbabilities,
} from "./persona-probabilities.js";
import { computeCalibrationRankingSplit, calibrationRankingSplitCommitment } from "./calibration-ranking-split.js";
import {
...
import { registryMetadata } from "./citation-registry.js";
import { scanCouncilCoverage } from "./citation-scanner.js";
import { assessProxyRisk, proxySchemaMetadata } from "./proxy-detector.js";
...
export function runLoanCouncil(rawLoan) {
  ...
  voices.push({
    ...
    confidence: 0.82,
    probabilities: roundProbabilities(creditFundamentalsProbabilities(loan)),
    rationale: !fico_pass
  ...
  voices.push({
    ...
    confidence: 0.78,
    probabilities: roundProbabilities(riskOfficerProbabilities(loan, risk, risk_budget_status)),
  ...
  voices.push({
    ...
    confidence: 0.91,
    probabilities: roundProbabilities(fairLendingProbabilities(loan)),
  ...
  voices.push({
    ...
    confidence: 0.74,
    probabilities: roundProbabilities(customerAdvocateProbabilities(loan)),
    rationale: advocate_ok
      ? "Customer-facing explanation and adverse-action reason list reviewed against CFPB Circular 2026-03 plain-language readability standards; no principal-reason accuracy or dictionary coverage flags."
      : `Adverse-action reasons present (${loan.adverse_action_reasons.length}); each reason must satisfy CFPB Circular 2026-03 model-traceability + Circular 2022-03 principal-reason accuracy — 'internal standards' or broad-bucket phrasing is insufficient. Escalate for advocate review of borrower-facing wording.`,
  ...
  voices.push({
    ...
    confidence: 0.69,
    probabilities: roundProbabilities(macroContrarianProbabilities(loan)),
  ...
  // Calibration/ranking split (arXiv:2605.27712): the single aggregated_score
  // collapses "how confident" and "how approvable" into one number, so two loans
  // with very different signal can land on the identical score (the documented
  // obvious_approve == borderline_escalate == 0.6575 defect). Expose them orthogonally
  // + a commitment so the pair is attestable and a silent swap breaks verification.
  const splitVoices = voices.map((v) => ({ voice_name: v.voice, verdict: v.verdict, confidence: v.confidence }));
  const calSplit = computeCalibrationRankingSplit(splitVoices);
  ...
  const response = {
    ...
    calibration_ranking_split: {
      calibrated_p: calSplit.calibrated_p,
      ranking_score: calSplit.ranking_score,
      commitment_sha256: calibrationRankingSplitCommitment(calSplit, splitVoices),
    },
    voice_contributions: cw.voice_contributions,
    aggregation_method: cw.aggregation_method,
    // Confidence semantics (product-facts.json → confidence_semantics). The per-voice
    // `confidence` numbers (0.82 / 0.78 / 0.91 / 0.74 / 0.69) are FIXED persona priors —
    // "stance strength", NOT a model probability and NOT a probability-of-correctness.
    // A reliability diagram cannot be drawn from these constants. UIs / PPT / JSON MUST
    // render them under this semantics, never as "model confidence" / "82% correct".
    // Kept under the field name `confidence` for backward compatibility.
    confidence_semantics: {
      ...
      note: "Fixed per-persona priors from the deterministic (rule-based) council; not calibrated model output. Overall resolved-outcome n<100 — no production coverage claim.",
    },
    ...
    // v1.5.18: citation registry binding. registryMetadata() emits
    // {version, entry_count, alias_count, verified_count, sunset_count,
    // registry_sha256}. Bank counsel pins registry_sha256 in the
    // procurement contract; post-hoc registry edit breaks Ed25519
    // attestation verification the same way v1.5.8 dictionary_hash does.
    citation_registry: registryMetadata(),
    // v1.5.18: per-rationale citation coverage — advisory. Bank auditor
    // sees which voices cite registry-resolved sections vs which cite
    // language the registry couldn't resolve. NOT REWORK-blocking yet
    // (v1.5.18 collects baseline); promote to REWORK in v1.5.19 after
    // per-persona base rates are known.
    citation_check: scanCouncilCoverage(voices),
    // v1.5.19: proxy-risk assessment. Direct-mention ECOA §701 terms
    // in rationale prose are hard-block; combinatorial signals ship as
    // advisory FLAG only per honest scope (Fed itself has no crisp
    // solution to combinatorial proxy detection). Bank counsel pins
    // proxy_schema_sha256 in procurement contract; post-hoc edits to
    // the ECOA blocklist break Ed25519 verification.
    proxy_risk_assessment: assessProxyRisk({
      voices,
      applicant_features: loan.applicant_features ?? null,
      jurisdiction: loan.jurisdiction ?? "US-ECOA",
    }),
    proxy_schema: proxySchemaMetadata({
      jurisdiction: loan.jurisdiction ?? "US-ECOA",
    }),
  ...
  // Required by BRD Governance Controls + SR 26-2 (formerly SR 11-7) invariants.
