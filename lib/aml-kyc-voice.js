...
// same ship. Under CFPB Circular 2026-03 model-traceability, AML
...
import { amlKycProbabilities, roundProbabilities } from "./persona-probabilities.js";
...
export function computeAmlKycVoice(loan) {
  ...
  const probabilities = roundProbabilities(amlKycProbabilities({
    hardBlockCount: findings.filter((f) => f.tier === "block").length,
    escalateCount: findings.filter((f) => f.tier === "escalate").length,
  }));
