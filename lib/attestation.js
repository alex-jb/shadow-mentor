...
function _signingPayload({ mode, requestCommitment, outputCommitment,
  ...
  // Only present when the caller bound a citation registry hash into
  // the signature. Pre-v1.5.18 attestations verify unchanged. Post-
  // 2026-07-07 attestations bind the citation registry SHA-256 so a
  // post-hoc registry edit (adding hallucinated CFR sections, editing
  // verbatim snippets, changing valid_for_aa_codes) breaks Ed25519
  // verification the same way v1.5.8 dictionary_hash does for the
