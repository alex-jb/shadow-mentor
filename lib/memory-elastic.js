...
// Names the backend buildMemoryBackend() will return for the CURRENT env, so an
// endpoint can report it honestly without re-deriving the env logic. `persistent`
// = survives a process restart (banks care: an ephemeral demo backend must not be
// mistaken for a durable audit store). Keep the branch order identical to the
// factory above — same env → same answer.
export function describeMemoryBackend() {
  if (process.env.SHADOW_MEMORY_BACKEND === "tiered" && process.env.SHADOW_MEMORY_DIR) {
    return { name: "local-tiered", persistent: true };
  }
  if (process.env.SHADOW_MEMORY_BACKEND === "elastic" && process.env.ELASTIC_URL) {
    return { name: "elasticsearch", persistent: true };
  }
  return { name: "in-memory-mock", persistent: false };
}
