...
export const SIVE_FIXTURES = Object.freeze([
  {
    ...
    expected_verdict: "approve",  // 2026-07-09 v1.5.44: SIVE Finding #1 RESOLVED
    valence: 1.0,
    loan: {
      credit_score: 780,
      debt_to_income: 0.20,
      loan_to_value: 0.60,
      amount: 1_500_000,
      // v1.5.44: switched from commercial_real_estate → consumer.
      // The Macro Contrarian persona is by design ALWAYS-escalate on
      // CRE per Lora's late-cycle regime policy; that isn't a bug in
      // the persona, it's a bug in the fixture calling itself
      // "obvious approve" while carrying a sector whose persona
      // structurally cannot approve. SIVE says: characterize the
      // instrument's response function. This fixture now embodies the
      // ACTUAL "obvious approve" shape — clean fundamentals AND a
      // sector every persona can approve.
      sector: "consumer_discretionary",
      borrower_rating: "AA",
      // v1.5.44: favorable market-proxy price series so historical
      // VaR@95%/10d lands well below the 12% Addendum C ceiling.
      // Previously fell through to normalizeLoan's synthetic stressed
      // sequence which structurally escalated the Risk Officer voice.
      market_proxy_prices: [
        100.0, 100.3, 100.5, 100.4, 100.7,
        100.9, 101.1, 101.0, 101.3, 101.5,
        101.6, 101.8, 102.0, 102.1, 102.3,
      ],
    },
    why:
      "FICO well above 700 floor, DTI well below 0.36 ceiling, LTV " +
      "well below 0.80 ceiling, consumer sector (no CRE regime " +
      "escalation), low-volatility uptrend prices (VaR below Addendum " +
      "C ceiling). All 5 personas structurally clear their approve " +
      "gates → obvious approve.",
  },
  {
    name: "obvious_deny",
