...
import {
  verifyPackageDir, MEMBER_PATHS, PACKAGE_VERSION, PACKAGE_VERSION_1_1, PACKAGE_VERSION_1_2,
  SUPPORTED_PREDECESSOR_VERSIONS,
} from "./portable-audit-package.mjs";

// versions whose signed supersedes block defines a chain link (1.0 never links)
const SUCCESSOR_VERSIONS = Object.freeze([PACKAGE_VERSION_1_1, PACKAGE_VERSION_1_2]);
...
export function verifyPackageChain(dirs, { publicKeyPem = null, requireCompleteChain = true } = {}) {
  ...
  // 3 · link resolution from signed claims (successors are 1.1 and 1.2 packages;
  // the 1.2 decision marker stays semantics-free HERE — decision lifecycle is a
  // separate derived view, never chain-link logic)
  const links = [];
  for (const e of entries) {
    if (!SUCCESSOR_VERSIONS.includes(e.manifest_version) || !e.supersedes) continue;
